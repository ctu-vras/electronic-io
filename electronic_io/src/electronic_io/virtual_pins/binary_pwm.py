...
class BinaryPWM(VirtualPin):
    ...
    def from_dict(pin_name, config, io_board):
        ...
        pin_info.pin.is_readable = bool(config.get("is_readable", pin.is_readable))
        pin_info.pin.is_writable = bool(config.get("is_writable", pin.is_writable))
        pin_info.pin.can_persist = bool(config.get("can_persist", pin.can_persist))

        return BinaryPWM(name, pin_info, on_threshold, readback_on_value, readback_off_value)
